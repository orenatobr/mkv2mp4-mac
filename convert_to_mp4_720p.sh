...
# - MP4: just re-encode to 720p, no track filtering.
    ...
    case "$EXT_LOWER" in
        mkv)
            ...
            | awk -F',' 'BEGIN{IGNORECASE=1} $2 ~ /^(por|pt|pt-br)$/ {print $1; exit}' || true)"
                ...
                | awk -F',' 'BEGIN{IGNORECASE=1} {idx=$1; c=$2; lang=$3; if (lang ~ /^(por|pt|pt-br)$/ && c ~ /^(ass|ssa|subrip|srt|text)$/) {print idx; exit}}' || true)"
